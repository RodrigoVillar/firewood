...
pub trait DynDb: Debug + Send + Sync + 'static {
    ...
    fn reconstruct_from_view(
        &self,
        parent: &std::sync::Arc<
            firewood_storage::NodeStore<firewood_storage::Committed, firewood_storage::FileBacked>,
        >,
        batch: OwnedBatch,
    ...
    fn reconstruct_from_reconstructed(
        ...
        parent: std::sync::Arc<
            firewood_storage::NodeStore<
                firewood_storage::Reconstructed<firewood_storage::FileBacked>,
                firewood_storage::FileBacked,
            >,
        >,
        batch: OwnedBatch,
    ) -> Result<crate::db::ReconstructedView<'_>, Error>;

    /// Get the concrete committed revision for `hash`, if this database runs
    /// in the build's default hash mode.
    ///
    /// Reconstruction is `DefaultHashMode`-only at the storage layer, so this
    /// returns the concrete committed nodestore needed to seed it. A database
    /// opened in a non-default mode returns `Ok(None)`; the reconstruct caller
    /// then surfaces [`Error::FeatureNotSupported`].
    #[expect(clippy::missing_errors_doc)]
